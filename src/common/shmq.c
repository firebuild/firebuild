...
bool shmq_reader_has_message(shmq_reader_t *reader) {
  ...
  int32_t header_location =
      ((shmq_next_message_pointer_t *)(reader->buf + tail_location))->next_message_location;
...
int32_t shmq_reader_get_message(shmq_reader_t *reader, const char **message_body_ptr) {
  ...
  /* Maybe the message body (mb[3] in the example) or its next pointer (p[3]) isn't mapped yet. */
...
void shmq_reader_maybe_send_early_ack(shmq_reader_t *reader) {
  ...
  shmq_global_header_t *global_header = (shmq_global_header_t *)(reader->buf);

  if (reader->ack_needed && !reader->ack_sent) {
    global_header->ack_seq = reader->seq;
    global_header->ack_count++;
    sem_post(&global_header->ack_sem);
...
void shmq_reader_message_done(shmq_reader_t *reader) {
  ...
  /* The location of p[2] according to the example. */
  int32_t tail_location = global_header->tail_location;
  assert(tail_location % 8 == 0);

  /* Where p[2] points to, i.e. the location of mh[3] according to the example. */
  int32_t message_location =
      ((shmq_next_message_pointer_t *)(reader->buf + tail_location))->next_message_location;
   ...
   * p[2], mh[3] and mb[3]. */
  global_header->tail_location = message_location + shmq_message_header_size() + roundup8(len);
...
void shmq_writer_init(shmq_writer_t *writer, const char *name) {
  ...
  shmq_global_header_t *global_header = (shmq_global_header_t *)(writer->buf);

  ((shmq_next_message_pointer_t *)(writer->buf + shmq_global_header_size()))->
      next_message_location = -1;
  global_header->tail_location = shmq_global_header_size();
  sem_init(&global_header->ack_sem, 1, 0);
  global_header->ack_count = 0;
  global_header->ack_seq = 0;
...
void shmq_writer_fini(shmq_writer_t *writer) {
  shmq_global_header_t *global_header = (shmq_global_header_t *)(writer->buf);
  sem_destroy(&global_header->ack_sem);
...
/**
 * Find a place for a new message of len bytes, which then can be constructed here in place by the
 * caller.
 */
...
#if 0  // complicated and currently not needed
/**
...
char *shmq_writer_resize_message(shmq_writer_t *writer, int32_t len) {
  ...
  if (len <= writer->next_message_len) {
    ...
    // TODO(egmont) We could check if we can grow in place, to avoid a memmove(). Ideally we would
    // advance the tail first, to increase the chance of this. The logic would get very complicated,
...
void shmq_writer_send_message(shmq_writer_t *writer, bool ack_needed) {
  ...
  ((shmq_message_header_t *)(writer->buf + writer->next_message_location))->len =
      writer->next_message_len;
  ...
  ((shmq_next_message_pointer_t *)(writer->buf + writer->next_message_location +
                                   shmq_message_header_size() +
                                   roundup8(writer->next_message_len)))->next_message_location = -1;

  /* Link it up from the previous message, so that the reader can see it. */
  ((shmq_next_message_pointer_t *)(writer->buf +
                                   writer->chunk[shmq_writer_nr_chunks(writer) - 1].head -
                                   shmq_next_message_pointer_size()))->next_message_location =
  ...
  if (writer->next_state != writer->state) {
    ...
    writer->chunk[writer->state].head =
        writer->next_message_location + shmq_message_overall_size(writer->next_message_len);
  } else {
    /* No state change, append to the head chunk. */
    writer->chunk[shmq_writer_nr_chunks(writer) - 1].head +=
        shmq_message_overall_size(writer->next_message_len);
  }
 ...
 * Wait for ACK.
 *
 * Must be called exactly once for each message that was sent with ack_needed=true, before starting
 * to construct the next message.
 */
void shmq_writer_wait_for_ack(shmq_writer_t *writer) {
  ...
  int ret;
  do {
    ret = sem_wait(&global_header->ack_sem);
  } while (ret == -1 && errno == EINTR);

  writer->ack_recv_count++;
  assert(global_header->ack_seq == writer->seq);
  assert(global_header->ack_count == writer->ack_recv_count);
}
